...
const emit = defineEmits<{(e: 'pageEvent', props: any): void,
    ...
    (e: 'viewEvent', game: GameData): void,
    (e: 'downloadEvent', game: GameData): void,
    (e: 'update:pagination', pagination: Pagination): void,
    (e: 'openEvent', game: GameData, site: string, newTab: boolean): void,
  }>();
...
const visibleColumns = ref(
  LocalStorage.getItem('visibleColumns') as string[]
...
function formatRatingChange(change: number) {
  ...
  const c1 = Math.floor(change / 10).toString();
  const c2 = (change % 10).toString();
  return `${sign}${c1}.${c2}`;
}

function generateRatingString(game: GameData, color: 'white' | 'black') {
  const gametime = game.date / 1000;
  const abandonedtime = Date.now() - 3600 * 6;
  if (game.size < 5
    || game.unrated === 1 || game.rating_white === 0 || game.rating_black === 0
    || (game.result !== '0-0' && game.notation.length < 6)
    || (game.result === '0-0' && gametime < abandonedtime)
    || (game.rating_change_white === -2000 && game.rating_change_black === -2000)
  ) {
...
function handleCopyPTN(game: GameData) {
  emit('copyEvent', game);
}

function handleViewPTN(game: GameData) {
  emit('viewEvent', game);
}

function handleDownload(game: GameData) {
