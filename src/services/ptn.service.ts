...
export class PTNService {
  private static getHeader(key: string, val: string|number) {
    return `[${key} "${val}"]\n`;
  }

  private static convertMove(move: string) {
  ...
  private static getMoves(notation: string) {
    let moves = '';
    let count = 0;
  ...
  private static getTimerInfo(timertime: number, timerinc: number) {
  ...
  public static getPTN(game: GameData) {
    let ptn = '';
    ...
    const [dateString, timeString] = new Date(game.date).toISOString()
      .split('.')[0].split('T');

    ptn += this.getHeader('Date', dateString.replaceAll('-', '.'));
    ptn += this.getHeader('Time', timeString);
